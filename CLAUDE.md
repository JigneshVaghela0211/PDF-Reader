...
- **Text selection over rendered pages** is a custom gesture bridge (`TextSelectionOverlay`/`TextSelectionToolbar`) mapping touch bounds to PDFBox `TextWord` rects — there is no native OS text selection on rendered bitmaps. Selection is **word-level**: `PdfTextBlockExtractor` splits words on whitespace (not just glyph-gap), `PdfWordHitTester` maps a touch to the word under it, and a long-press selects a single word. `TextSelectionState` carries `startWord`/`endWord` anchors; two `SelectionHandle`s drive `PdfEditorViewModel.moveSelectionStart`/`moveSelectionEnd`, which delegate range math to `domain/usecase/SelectionRangeUseCase`. The handles can cross over each other (range is order-independent).
